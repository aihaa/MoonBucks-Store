...
file = pd.read_csv(r'C:\Users\Aiman Fatihahh\Downloads\WIA2005-Problem2\dataset-starbucks_2018_11_06.csv', delimiter =',', index_col="state")
...
def localDistributor(stateName):    
    ...
    for i in range(len(data)): # tukar origin
        sum = 0
        origin = data.iloc[[i]]
        for j in range(len(data)): # tukar TO distance
           # originFrom = origin.latitude,origin.longitude
           # originTo = data.iloc[[j]].latitude,data.iloc[[j]].longitude
            x1 = origin.latitude
            y1 = data.iloc[[j]].latitude
            x2 = origin.longitude
            y2 = data.iloc[[j]].longitude
            sum1 = distance(x1,y1,x2,y2)
           # sum1 = distance(origin.street_address,data.iloc[[j]].street_address)
            sum += sum1
